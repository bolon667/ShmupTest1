...
void playerInputChanged() {
	...
	//We only read data from the joypad 1
		...
		if((input.state & BUTTON_C) && !(input.prevState & BUTTON_C)){
			...
			//          |
			//Fun thing V
			//addNew_Boss1Bullet(pos, (Vect2D_f16){FIX32(4),0});
		...
		if((input.state & BUTTON_START) && !(input.prevState & BUTTON_START)){
			
			stopLevel();
			PAL_setColors(0, img_pauseBG.palette->data, 16, DMA);
			VDP_drawImageEx(BG_A, &img_pauseBG, TILE_ATTR_FULL(PAL0, FALSE, FALSE, FALSE, VDPTilesFilled), 0, 0, FALSE, FALSE);
			while(1){
				input.prevState = input.state;
				input.state = JOY_readJoypad(JOY_1);
				if((input.state & BUTTON_START) && !(input.prevState & BUTTON_START)){
			...
			//loading level, palette, music, tilesets, and reloading sprites on screen
			resumeLevel();
		}
